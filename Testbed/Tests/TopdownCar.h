...
#ifndef TOPDOWNCAR2_H
#define TOPDOWNCAR2_H
...
public:
    ...
    void updateDrive(int controlState) {
        ...
        if ( lateralFrictionImpulse.Length() > lateralImpulseAvailable )
            lateralFrictionImpulse *= lateralImpulseAvailable / lateralFrictionImpulse.Length();
        m_body->ApplyLinearImpulse( m_currentTraction * lateralFrictionImpulse, m_body->GetWorldCenter(), true );
    ...
    /*void updateTurn(int controlState) {
        ...
        switch ( controlState & (TDCR_LEFT|TDCR_RIGHT) ) {
            case TDCR_LEFT:  desiredTorque = 15;  break;
            case TDCR_RIGHT: desiredTorque = -15; break;
            default: ;//nothing
        }
...
public:
    TopdownCar()
    {
        //replace the testbed's default world with the one saved in json file
        {
            b2dJson json;
            std::string err_msg;
            b2World* world = json.readFromFile("../Testbed/Data/racetrack.json", err_msg);
            if ( world ) {
                delete m_world;
                m_world = world;
	...
	void Keyboard(int key)
	{
		switch (key)
		{
        case GLFW_KEY_A : m_controlState |= TDCR_LEFT; break;
        case GLFW_KEY_D : m_controlState |= TDCR_RIGHT; break;
        case GLFW_KEY_W : m_controlState |= TDCR_UP; break;
        case GLFW_KEY_S : m_controlState |= TDCR_DOWN; break;
    ...
    void Step(Settings* settings)
    {
        ...
        b2Vec2 oldViewCenter = g_camera.m_center;
        b2Vec2 posOfCarVerySoon = m_car->m_body->GetPosition() + 0.25f * m_car->m_body->GetLinearVelocity();
        g_camera.m_center = 0.9f * oldViewCenter + 0.1f * posOfCarVerySoon;

        //show some useful info
        g_debugDraw.DrawString(5, m_textLine, "Press w/a/s/d to control the car");
        m_textLine += 15;

        g_debugDraw.DrawString(5, m_textLine, "Speed: %.2f", m_car->getForwardVelocity().Length());
        m_textLine += 15;
